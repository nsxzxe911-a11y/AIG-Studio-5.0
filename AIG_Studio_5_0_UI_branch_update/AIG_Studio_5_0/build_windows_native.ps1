...
$Desktop = Join-Path $Root 'desktop\src\main\kotlin\com\aigstudio\desktop\DesktopApp.kt'
...
$Sources = @(Get-ChildItem $CoreDir -Filter '*.kt' | Sort-Object Name | ForEach-Object { $_.FullName })
$Sources += $Desktop
...
& jpackage --type exe --name $Product --dest $Out --input $Dist --main-jar (Split-Path -Leaf $Jar) --main-class com.aigstudio.desktop.DesktopAppKt --app-version 5.12.0 --vendor 'AIG' --description 'AIG Studio 5.0 RGB CNC Workstation' --win-upgrade-uuid $UpgradeUuid --win-dir-chooser --win-shortcut --win-menu --win-menu-group 'AIG'
