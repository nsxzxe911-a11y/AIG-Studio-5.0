...
object AiCadInspector {
    fun inspect(snapshot: DrawingSnapshot): AiInspection {
        ...
        entities.forEach { e ->
            when (e) {
                is Line -> if (e.length <= 0.001) issues += AiIssue(IssueSeverity.DANGER, "SHORT_LINE", "極短線/零長度線，CAM 前必須確認", listOf(e.id))
                is Circle -> if (e.radius <= 0.001) issues += AiIssue(IssueSeverity.DANGER, "TINY_CIRCLE", "圓半徑過小，CAM 前必須確認", listOf(e.id))
                is Arc -> if (e.radius <= 0.001 || e.start.distanceTo(e.end) <= 0.001) issues += AiIssue(IssueSeverity.DANGER, "TINY_ARC", "圓弧尺寸異常，CAM 前必須確認", listOf(e.id))
            }
        }
        ...
        for (i in endpoints.indices) for (j in i + 1 until endpoints.size) {
            ...
            if (d > 1e-6 && d <= 0.01) issues += AiIssue(IssueSeverity.WARNING, "NEAR_GAP", "端點非常接近但未連接（${"%.4f".format(d)}）", listOf(id1, id2))
        }
    ...
    private fun sameGeometry(a: Entity, b: Entity): Boolean = when {
        ...
        a is Circle && b is Circle -> near(a.center,b.center) && abs(a.radius-b.radius) <= 1e-6
        else -> false
    }
    private fun near(a: Vec2, b: Vec2) = a.distanceTo(b) <= 1e-6
}
